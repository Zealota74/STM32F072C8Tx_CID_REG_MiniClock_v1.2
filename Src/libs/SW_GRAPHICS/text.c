...
#include "fb.h"
#include "SW_HT1632C/sw_ht3216c.h"
#include "gfx.h"
...
static const uint16_t _width  = SCREEN_WIDTH;
static const uint16_t _height = SCREEN_HEIGHT;
...
INLINE void draw_H_byte( volatile T_DISPLAY *buffer, int16_t x, int16_t y, uint8_t byte,
						 T_COLOR color, const T_GAMMA *gamma ) {
	T_RGB_2ROWS colorTmp[ 8 ] = { {0}, {0}, {0}, {0}, {0}, {0}, {0}, {0} };
	graphic_24bit_to_rgb( color, colorTmp, gamma );

//	if(0) {
//		if ( 0x80 & byte ) fb_set_pixel_RAM( buffer, x + 0, y, colorTmp );
//		else			   fb_set_pixel_RAM( buffer, x + 0, y, colorBlack );
//		if ( 0x40 & byte ) fb_set_pixel_RAM( buffer, x + 1, y, colorTmp );
//		else			   fb_set_pixel_RAM( buffer, x + 1, y, colorBlack );
//		if ( 0x20 & byte ) fb_set_pixel_RAM( buffer, x + 2, y, colorTmp );
//		else			   fb_set_pixel_RAM( buffer, x + 2, y, colorBlack );
//		if ( 0x10 & byte ) fb_set_pixel_RAM( buffer, x + 3, y, colorTmp );
//		else			   fb_set_pixel_RAM( buffer, x + 3, y, colorBlack );
//		if ( 0x08 & byte ) fb_set_pixel_RAM( buffer, x + 4, y, colorTmp );
//		else			   fb_set_pixel_RAM( buffer, x + 4, y, colorBlack );
//		if ( 0x04 & byte ) fb_set_pixel_RAM( buffer, x + 5, y, colorTmp );
//		else			   fb_set_pixel_RAM( buffer, x + 5, y, colorBlack );
//		if ( 0x02 & byte ) fb_set_pixel_RAM( buffer, x + 6, y, colorTmp );
//		else			   fb_set_pixel_RAM( buffer, x + 6, y, colorBlack );
//		if ( 0x01 & byte ) fb_set_pixel_RAM( buffer, x + 7, y, colorTmp );
//		else			   fb_set_pixel_RAM( buffer, x + 7, y, colorBlack );
//	} else {
		if ( 0x80 & byte ) fb_set_pixel_RAM1( buffer, x + 0, y, colorTmp );
		if ( 0x40 & byte ) fb_set_pixel_RAM1( buffer, x + 1, y, colorTmp );
		if ( 0x20 & byte ) fb_set_pixel_RAM1( buffer, x + 2, y, colorTmp );
		if ( 0x10 & byte ) fb_set_pixel_RAM1( buffer, x + 3, y, colorTmp );
		if ( 0x08 & byte ) fb_set_pixel_RAM1( buffer, x + 4, y, colorTmp );
		if ( 0x04 & byte ) fb_set_pixel_RAM1( buffer, x + 5, y, colorTmp );
		if ( 0x02 & byte ) fb_set_pixel_RAM1( buffer, x + 6, y, colorTmp );
		if ( 0x01 & byte ) fb_set_pixel_RAM1( buffer, x + 7, y, colorTmp );
//	}
}

/*********************************************** VERSJA2 *************************************************/
INLINE uint8_t graphic_draw_char_RAM( volatile T_DISPLAY *buffer, int16_t x, int16_t y, CHAR * charCode, uint32_t  color,
									  T_COLOR bg, uint8_t drawLine, FONT_INFO *fontPtrPGM, const T_GAMMA *gamma ) {
...
static inline void draw_H_pixel_bits( volatile T_DISPLAY * buffer, int16_t x, int16_t y,
									 uint8_t byte, T_COLOR color, uint8_t width ) {
...
//static inline void draw_H_pixel_byte( volatile T_DISPLAY * buffer, int16_t x, int16_t y,
//									 uint8_t byte, T_COLOR color, uint8_t width ) {
//}
static uint8_t graphic_draw_char_RAM( volatile T_DISPLAY *buffer, int16_t x, int16_t y, CHAR *charCode, T_COLOR color,
									  T_COLOR bg, uint8_t drawLine, FONT_INFO *fontPtrPGM, const T_GAMMA *gamma ) {
	uint8_t	transCode = polish_letters_UTF8( charCode );
	if ( transCode == UTF8_HIGH_BYTE ) return 0;

	uint16_t charOffset;
	uint8_t	 charWidthBits;
	uint8_t	 charHeight		  = fontPtrPGM->heightPixels;
	uint8_t	 charAddressStart = fontPtrPGM->startChar;
	uint8_t  size = 1;
	...
	if ( transCode == ASCII ) {
		...
		charOffset		= fontPtrPGM->charInfo[ fontPtrPGM->sizeArray -
		          		 (PL_CHARACTERS_COUNT - transCode) ].offset;
	}
	if ( *charCode == SPACE ) {
	...
	for ( uint8_t iY = 0; iY < charHeight; iY++ ) {		// Next horizontal line
		for ( uint8_t jX = 0; jX < horByteNum; jX++ ) {	// Next column
			...
			draw_H_pixel_bits( buffer, x, y + iY, byte, color, charWidthBits );
		} // Columns end
	} // Rows end
	return charWidthBits;
}
...
/********************* Wystawiam liczby na ekran, zwracam długość string'a w pikselach ******/
...
//static T_DISPLAY * TextBuffer = ActualBuffer1;
...
void TEXT_display_number( int16_t x, int16_t y, int16_t number, T_STRING * Text ) {
	...
	graphic_puts_int_RAM( noActiveBuffer, Text, number, FONTx1, WhiteColor, BlackColor, &GammaRGB );
}
void TEXT_display_float( int16_t x, int16_t y, float number, T_STRING * Text ) {
	...
	Text->onChange = 1;
	Text->fontPtrPGM = (FONT_INFO *)&MicrosoftSansSerif8ptFontInfo_var;
	graphic_puts_float_RAM( noActiveBuffer, Text, number, FONTx1, WhiteColor, BlackColor, &GammaRGB );
}
void TEXT_display_string( int16_t x, int16_t y, CHAR * str, T_STRING * Text ) {
	...
	Text->str = (CHAR *)str;
	Text->fontPtrPGM = (FONT_INFO *)&MicrosoftSansSerif8ptFontInfo_var ;
	graphic_puts_RAM( noActiveBuffer, Text, FONTx1, WhiteColor, BlackColor, &GammaRGB );
}
...
void test_text_display_number( uint8_t number ) {
	...
	graphic_puts_RAM( 	  TextBuffer, &Text1, FONTx1, 		  (uint32_t)WhiteColor, BlackColor, &GammaRGB );
	graphic_puts_int_RAM( TextBuffer, &Text2, number, FONTx1, (uint32_t)WhiteColor, BlackColor, &GammaRGB );
}
/********************************************************************************************/
...
//	b = graphic_puts_RAM( buffer, &Text1, FONTx1, MagentaColor, BlackColor, &GammaRGB );
}
/********************************************************************************************/
...
void text_scrolling_text( volatile T_DISPLAY * buffer, T_COLOR color, int8_t dir ) {
	...
	static uint16_t b = 0;
//	if (b==0) {
		b = graphic_puts_RAM( buffer, &Text1, FONTx1, color, BlackColor, &GammaRGB );
//	}
